...
[STATWORX](https://www.statworx.com/en/), I helped to establish a company wide
...
mtcars %>% 
  filter(cyl >= 6) %>% 
  mutate(`l/100km` = mpg / 235.2 * 100)

rbindlist(list(mtcars, mtcars)) %>% 
...
mtcars %>% 
  dplyr::filter(cyl >= 6) %>% 
  dplyr::mutate(`l/100km` = mpg / 235.2 * 100)

data.table::rbindlist(list(mtcars, mtcars)) %>% 
